...
public class chooseCitySpeechlet implements Speechlet {
    ...
    /**
     * Database word gebruikt voor alle com.database handelingen, de unique user id is het ID van de gebruiker van de echo.
     * Deze word ingesteld op het moment dat de skill gestart word.
     * De NSIApi word gebruikt voor alle api requests aan de NSI Api
     */
    ...
    /**
     * Op het moment dat de gebruiker 3 ritjes aan hem gepresenteerd krijgt word deze boolean true, zodat de chooseintent kan worden
     * aangeroepen.
     */
    ...
    /**
     * Deze functie word als eerste aangeroepen als de skill word geactiveerd, meeste hiervan is geschreven door Amazon.
     * Maar ik stel hier ook de huidige tijd en datum in zodat ik een request kan uitvoeren als dat nodig is.
     * Gebruiker word toegevoegd aan de com.database. Naam is een placeholder, kan worden aangepast als account linking word toegepast.
     * @param request
     * @param session
    ...
    /**
     * Deze functie word aangeroepen als de skill word gestart zonder intent.
     * De functie roept hierna getUpdatedResponse aan, deze zal de gebruiker op de hoogte brengen van eventuele veranderingen in zijn
     * geboekte reizen.
     * @param request
     * @param session
     * @return
    ...
    /**
     * Als de skill word aangeroepen met een intent, gebeurt dat hier. Er word gecheckt welke Intent het is, en daarna word de
     * bijbehorende functie aangeroepen.
     * @param request
     * @param session
     * @return
     * @throws SpeechletException
    ...
    /**
     * Deze functie word aangeroepen als de Traveler intent binnen komt. Alle gegevens worden ingevoerd, en een request word gestuurd
     * Hierna worden 3 mogelijke reis opties teruggegeven aan de gebruiker, als de reiziger heeft aangegeven dat het om vertrek gaat.
     * Dan worden de vertrektijden gegeven, maar default is aankomsttijden.
     * @param intent
     * @return
     */
    ...
    /**
     * Deze functie word aangeroepen als de Chooseintent intent binnenkomt, en de boolean JourneySelected true is. Hier word extra
     * informatie over de reis teruggegeven aan de gebruiker, en de bijbehorende data opgeslagen in de Database.
     * @param intent
     * @return
     */
    private SpeechletResponse getChooseIntentResponse(Intent intent){
        ...
        if(option != null && option.getValue() != null){
            try{
                ...
                return newAskResponse("I didn't quite get that, which option did you choose?", "To cancel the order, just say exit");
    ...
    private SpeechletResponse getUpdatedResponse(){
        ...
        if(reizen.size() != 0) {
            PriceAndTimeRequest request = new PriceAndTimeRequest(reizen.get(0).getOriginCode(), reizen.get(0).getDestinationCode(), reizen.get(0).getDeparturedate(), reizen.get(0).getDeparturetime());
            Connections reis = API.getResponse(request);
            if (reis.getData().getConnections().get(0).getDuration().getDelay()) {
                altSpeech += "Your trip to " + reis.getData().getConnections().get(0).getDestination().getName() + " has been delayed";
            } else {
                altSpeech += "Your trip to " + reis.getData().getConnections().get(0).getDestination().getName() + " is going according to plan";
            }
        }else {
    ...
    /**
     * Deze functie word aangeroepen met de LocationIntent intent. En slaat een locatie op onder de identifier die de gebruiker aangaf
     * Nu gelimiteerd tot home en work. Station word opgezocht aan de hand van de opgegeven stad.
     * @param intent
     * @return
     */
    private SpeechletResponse getLocationIntentResponse(Intent intent){
        ...
        try{
            ...
            speech.setText("Location adding failed, you can try again if you want, or tell me where you want to travel");
        }
        repromptText.setText("If you want to quit you can say exit, or you can say I want to travel to Amsterdam");
        reprompt.setOutputSpeech(repromptText);
        return SpeechletResponse.newAskResponse(speech,reprompt);
    }

    /**
     * De functie word aangeroepen met de CompositionIntent intent. En slaat de gezinscompositie op. Dit is een aantal, en zijn
     * altijd adults. In de toekomst kan je hier nog andere age types aan toevoegen.
     * @param intent
     * @return
     */
    ...
    /**
     * Deze functie word aangeroepen met de Cheapest intent. En zoekt de goedkoopste reisdatum + prijs op
     * voor de gegeven origin + destination.
     * Je kan de functie aanroepen met alleen een destination, de functie kiest dan Amsterdam Centraal als origin.
     * @param intent
     * @return
     */
     ...
     * @param card
     *            the card to be shown to the user on the alexa web interface
    ...
    private Image createDestinationImage(){
        ...
        {
            case "Amsterdam":
                image.setSmallImageUrl("https://upload.wikimedia.org/wikipedia/commons/thumb/6/6d/Flag_of_Amsterdam.svg/1200px-Flag_of_Amsterdam.svg.png");
            ...
            default:
                ...
                image.setLargeImageUrl("https://www.malibuvista.com/wp-content/uploads/2015/04/iStock_000001947025_Full.jpg");
                break;
    ...
    private void initializeDateandTime(){
        ...
        date = "" + cal.get(Calendar.YEAR)+String.format("%02d",(cal.get(Calendar.MONTH) + 1))+String.format("%02d",cal.get(Calendar.DAY_OF_MONTH));
        time = "" + String.format("%02d",cal.get(Calendar.HOUR_OF_DAY) + 2) + String.format("%02d",cal.get(Calendar.MINUTE));
    }

    private SpeechletResponse newUser(){
        PlainTextOutputSpeech speechOutput = new PlainTextOutputSpeech();
        speechOutput.setText("Thank you for enabling this skill, since this is your first time, with how many people do you travel?");
